...
MODEL_DIR = MODELS_DIR / 'encoder_decoder_tuning'
...
def train_and_score_model(hidden_size, learning_rate, weight_decay, dropout, batch_size, trial_n):
    config = {
...
def objective(trial):
    hidden_size = trial.suggest_int("hidden_size", 40, 80)
    batch_size = 8
    learning_rate = trial.suggest_float("learning_rate", 1e-5, 1e-2, log=True)
    weight_decay = trial.suggest_float("learning_rate", 1e-5, 1e-2, log=True)
    dropout = trial.suggest_float("dropout", 0.2, 0.5)
    
    crps = train_and_score_model(hidden_size, learning_rate, weight_decay, 
