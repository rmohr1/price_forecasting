...
# CONFIGURATION
SAVE_DIR = "../../data/raw/caiso_demand_2DA"  # Directory to save cached files
...
def fetch_and_cache_demand(start_date, end_date, chunk_days=7):
    node = SystemDemand()  
    ...
    while dt < end_dt:
        ...
        cache_file = os.path.join(SAVE_DIR, f"2DA_Demand_{chunk_str}.csv")

        if os.path.exists(cache_file):
            print(f"[✓] Skipping cached chunk: {chunk_str}")
            df = pd.read_csv(cache_file, parse_dates=["INTERVALENDTIME_GMT"])
        else:
            print(f"[→] Fetching chunk: {chunk_str}")
            df = node.get_demand_forecast(dt, chunk_end_dt)
...
if __name__ == "__main__":
    df_full = fetch_and_cache_demand(START_DATE, END_DATE, CHUNK_DAYS)
